...
def get_return_and_rto_dates(data_dict: dict) -> dict:
    """
...
def add_quantity_in_sales(data_dict: dict) -> dict:
    """
...
def calculate_platform_discount(data_dict: dict) -> dict:
    """
    ...
    for file_type in file_types:
        df = data_dict[file_type]
        df["platform_funded_discount"] = (
            df["mrp"] - df["total_discount_amount"] - df["seller_product_amount"]
        )
        data_dict[file_type] = df
...
def reverse_entries_for_payment_reverse(data_dict: dict) -> dict:
    """
...
def add_tax_on_commissions(data_dict: dict) -> dict:
    """
...
def get_order_status(data_dict: dict) -> dict:
    """
    ...
    sales_df.loc[mask, "order_status"] = sales_df.loc[mask, "order_status"].map(
        rto_dict
    )
    mask = sales_df["order_status"].isna()
...
def create_single_payment_file(data_dict: dict) -> dict:
    """
...
def map_payments(data_dict: dict) -> dict:
    """
    ...
    sales["payment_status"] = sales["_merge"].replace(
        {"both": "settled", "left_only": "unsettled"}
    )
...
def get_returned_qty(data_dict: dict) -> dict:
    """
...
def calculate_net_delivered_quantity(data_dict: dict) -> dict:
    """
    Calculates net delivered quantity
    """

    mapped_orders = data_dict["mapped_orders"]
    mapped_orders["net_delivered_quantity"] = (
        mapped_orders["quantity"] - mapped_orders["returned_quantity"]
    )
    data_dict["mapped_orders"] = mapped_orders
...
def calculate_final_settlement(data_dict: dict) -> dict:
    """
...
def create_fee_estimator(data_dict: dict) -> dict:
    """
    ...
    fee_estimate_dict["commission"] = (
        settled.groupby("sku")["commission_pct"].mean().to_dict()
    )
...
def estimate_unsettled_orders(
    data_dict: dict, fee_estimate_dict: dict, complete_data_dict: dict
) -> dict:
    """
    ...
    for fee in fee_estimate_dict:
        if fee == "commission":
            all_sales["commission_pct"] = all_sales["sku"].map(
                fee_estimate_dict.get(fee)
            )
            all_sales[fee] = (
                all_sales["commission_pct"] * all_sales["net_realized_sale"]
            )
            all_sales.drop(columns=["commission_pct"], inplace=True)
        elif fee == "payment_gateway_fee":
            all_sales["pg_pct"] = all_sales["payment_status"].map(
                fee_estimate_dict.get(fee)
            )
...
def create_mis(data_dict: dict) -> dict:
    """
...
def create_payment_recon_rangita(data_dict: dict, complete_data_dict: dict) -> dict:
    """
    ...
    df.rename(
        columns={"order_date": "Order Date", "order_id": "Order No."}, inplace=True
    )
...
def map_myntra_payments(_path_: str) -> Tuple[Dict, Dict]:
    """
    ...
    Parameters:
    -----------
    _path_: Full path to the location where all the required files are there.

    Required files to be present in the directory:
    ----------------------------------------------
        Required files are (a) Forward payments (b) Reverse payments (c) GST sale report
        (d) GST REturns report (d) RTO files (e) Combined payment file.

        Each filename should contain the following "keywords" in their name, so that the
        code recognizes the file:

    |----------------------------------------------------
    |File type (All required)   |   Req keywords.       |
    |----------------------------------------------------
    |Forward payments           |   PG Forward          |
    |----------------------------------------------------
    |Reverse payments           |   PG Reverse          |
    |----------------------------------------------------
    |GST Sale report            |   GST Sales           |
    |----------------------------------------------------
    |GST Returns                |   GST RT              |
    |----------------------------------------------------
    |RTO files                  |   RTO                 |
    ...
    -------
    Two dictionaries:
        (a) data_dict: payment recon file will be called "mapped_orders".
    ...
    print(f"\n** IMPORTANT **\n---------------\n")
    print(
        f"Files need to contain certain keywords for python to understand the file type. See below:"
    )
    print(f"|----------------------------------------------------")
    ...
    data_dict = estimate_unsettled_orders(
        data_dict, fee_estimate_dict, complete_data_dict
    )
