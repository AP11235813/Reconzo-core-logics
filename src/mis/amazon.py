	...
	for file in folder_path.iterdir():
		if ".DS_Store" in file.name:
			...
			if any(x in file.stem for x in ["b2bReport", "b2cReport"]):
				...
				df["month_year"] = pd.to_datetime(df["month_year"], errors="coerce", dayfirst=True).dt.date
...
def tag_current_month_orders(
	...
	mask = (mtr["order date"] >= mtr["month_year"]) & (mtr["order date"] < mtr["month_year_end"])
...
def add_gross_sales(
	...
	mtr = data_dict["mtr"]
	asins = set(mtr["asin"])
	gross_sale_dict = {k: mtr[mtr["asin"] == k]["principal amount"].mode().iloc[0] for k in asins}
	mtr["gross_sale_mis"] = mtr["asin"].map(gross_sale_dict)
	data_dict["mtr"] = mtr

	return data_dict

def add_gross_quantity(
	data_dict: dict
	) -> dict:
	"""
	Adds a Gross sale number to each sale
...
def create_monthly_payments(
	...
	payments["posted-date-time"] = pd.to_datetime(payments["posted-date-time"], errors="coerce", dayfirst=True).dt.tz_convert("Asia/Kolkata")
	...
	for m in month_years:
		...
		mask = (payments["posted-date-time"] >= m) & (payments["posted-date-time"] < end_month)
		payments.loc[mask, "month_year"] = m
		payments.loc[mask, "month_year_end"] = end_month

	payments["month_year"] = pd.to_datetime(payments["month_year"], errors="coerce", dayfirst=True).dt.date
...
def create_mtr_pivots(
	...
	df = pd.pivot_table(temp_df, columns="month_year", index="key", values=col_to_pivot, aggfunc="sum").fillna(0)
	df = df.reset_index()
...
def create_mis_structure(
	...
	for status in statuses:
		...
		some_dict = {col: [temp_df[col].sum()] for col in temp_df.columns if col != "key"}
...
def tag_sku_and_orders(
	...
	mask = payments["order-id"].astype(str).str.contains(order_pattern1, regex=True) | payments["order-id"].astype(str).str.contains(order_pattern2, regex=True)
...
def get_list_order_costs(
	...
	unit_costs_non_tax = [cost for cost in unit_costs if not any(e in cost for e in exclusions)]
...
def create_monthly_orders_and_estimates(
	...
	payment_pivot = pd.pivot_table(order_payments, values="amount", index="key", columns="amount-description")
	...
	for m in set(mtr["month_year"]):
		mtr_order_payment = mtr[(mtr["month_year"] == m) & (mtr["transaction type"] == mtr_status)].copy()
		...
		mtr_order_payment = mtr_order_payment.merge(payment_pivot, on="key", how="left", indicator=True)
		...
		for col in cost_cols:
			mtr_order_payment[col] = mtr_order_payment[col] * total_orders / settled_orders
...
def add_monthly_unit_costs_to_mis(
	data_dict: dict
	) -> dict:
	"""
	"""
	...
	for m in monthly_unit_costs:
		dd[m] = {col: monthly_unit_costs.get(m)[col].sum() for col in monthly_unit_costs.get(m).columns if col in unit_costs}
...
def get_list_overhead_costs(
	...
	mask = ((~payments["amount-description"].isin(unit_costs)) & (~payments["amount-description"].isin(unit_tax_costs)))
	df = payments[mask].copy()
	...
	overhead_tax_costs = [cost for cost in overhead_costs if cost is not None and any(e in cost for e in tax_exclusions)]
	overhead_costs_non_tax = [cost for cost in overhead_costs if cost is not None and not any(e in cost for e in exclusions)]
...
def add_overheads_to_mis(
	...
	overhead_df = pd.pivot_table(payments, index="amount-description", columns="month_year", values="amount", aggfunc="sum")
...
def add_taxes(
	...
	exclusions = ["Product Tax", "Product tax discount", "TDS Reimbursement", "TDS (Section 194-O)"]

	tax_cols_all = unit_tax_costs + overhead_tax_costs
	tax_cols = [t for t in tax_cols_all if t is not None and not any(e in t for e in exclusions)]
	...
	df = payments[mask].copy()
	fee_tax_all = pd.pivot_table(df, index="amount-description", values="amount", columns="month_year", aggfunc="sum")
	fee_tax_mis = pd.DataFrame([fee_tax_all.sum()], index=["tax_on_fees"])

	mask = payments["amount-description"].isin(sales_tax_cols)
	df = payments[mask].copy()
	gst_tax_all = pd.pivot_table(df, index="amount-description", values="amount", columns="month_year", aggfunc="sum")
	...
	df = payments[mask].copy()
	tds_all = pd.pivot_table(df, index="amount-description", values="amount", columns="month_year", aggfunc="sum")
...
def create_amazon_mis(
	_path_: str='/Users/ap/Desktop/mis/amazon'
	) -> dict:
	"""
	Creates Amazon MIS
	...
	order_statuses = ["Net Delivered", "Return + RTO", "Cancelled", "Replacements", "Gross Sale"]
