...
def forward_fill_settlement_date(
	data_dict,
	cols_to_ffill: list=["settlement-start-date", "settlement-end-date", "deposit-date"]
	) -> dict:
	"""
...
def create_fee_table(
	...
	table = pd.pivot_table(filtered_pmt, index="key", columns="amount-description", values="amount", aggfunc="sum")
...
def create_referral_category_mapper(
	data_dict: dict
	) -> dict:
	"""
...
def clean_rate_card(
	...
	commission["lower"] = commission["Fee"].apply(extract_lower_bound_commission_price_bucket)
	commission["upper"] = commission["Fee"].apply(extract_upper_bound_commission_price_bucket)
	cols = ["rate", "lower"]
...
def calculate_commissions(
	...
	commissions_rate_card.columns=commissions_rate_card.columns.astype(str).str.lower().str.strip()
	date_cols = ["start date", "end date"]

	for col in date_cols:
		commissions_rate_card[col] = pd.to_datetime(commissions_rate_card[col], errors='coerce')
...
def force_orders_datetime_format(
	data_dict: dict
	) -> dict:
	"""
	...
	for col in cols:
		...
		df.loc[aware_mask, col] = pd.to_datetime(df.loc[aware_mask, col], errors='coerce').dt.tz_convert("Asia/Kolkata")
		df.loc[naive_mask, col] = pd.to_datetime(df.loc[naive_mask, col], errors='coerce').dt.tz_convert("Asia/Kolkata")
...
def calculate_shipping_weights(
	...
	weight_mapper["vol_weight"] = weight_mapper["l"] * weight_mapper["b"] * weight_mapper["h"] / 5
	cols = ["dead_weight", "vol_weight"]
	weight_mapper["shipping_weight"] = weight_mapper[cols].max(axis=1)
	shipping_weight_mapper = weight_mapper.set_index("asin")["shipping_weight"].to_dict()
...
def assign_weight_buckets(
	...
	"""
	table = data_dict["output"]
	table["shipping_weight_bucket"] = (table["shipping_weight"].fillna(0) / 500).fillna(0).astype(int) + 1
	mask = (table["shipping_weight_bucket"] > 2) & (table["shipping_weight_bucket"] % 2 == 1)
	table.loc[mask, "shipping_weight_bucket"] = table.loc[mask, "shipping_weight_bucket"] + 1

	data_dict["output"] = table
	
	return data_dict

def map_to_from_pincodes(
	data_dict
	) -> dict:
	"""
	...
	postcode_mapper.columns = postcode_mapper.columns.astype(str).str.strip().str.lower()
...
def map_zones(
	...
	table = data_dict["output"]
	mask = (table["from_region"].isna()) | (table["to_region"].isna()) | (table["from_city"].isna()) | (table["to_city"].isna())
...
def calculate_shipping_fee(
	...
	def process_table(table_subset: pd.DataFrame, model_name: str, rate_card: pd.DataFrame) -> pd.DataFrame:
		conditions = (
		...
		table_subset = table_subset.merge(rc, left_on="shipping_type", right_on="zone", how="left")
	...
	cols_to_drop = ["min weight", "max weight", "additional fee", "start date", "end date"]
...
def map_pick_and_pack_fee(
	...
	table = table.merge(rate_card, on="weight_bucket", how="left", suffixes=["", "_pnp"])
	...
	table.rename(columns={"fees per unit": "pick_and_pack_fee_per_unit_calc"}, inplace=True)
	cols = ["step level", "size band_pnp", "start date", "end date", "description", "min weight", "max weight"]
	table.drop(columns=cols, inplace=True)
	table["pick_and_pack_fee_calc"] = table["pick_and_pack_fee_per_unit_calc"] * table["quantity"].astype(int)
...
def create_fixed_closing_category_mapper(
	...
	if "category_mapper_fixed_closing" in data_dict:
		category_mapper_asins = list(set(data_dict.get("category_mapper_fixed_closing")["asin"]))
	...
	if "category_mapper_fixed_closing" in data_dict:
		...
		category_mapper_fixed_closing = pd.concat([category_mapper_fixed_closing, category_mapper_new], axis=0)
	else:
...
def get_fixed_closing_fees_categories(
	data_dict
	) -> dict:
	"""
	...
	category_mapper.columns = category_mapper.columns.astype(str).str.lower().str.strip()
...
def calculate_fixed_closing_fees(
	data_dict: dict
	) -> dict:
	"""
	"""
	...
	table = table.merge(rate_card, left_on="category_fixed_closing", right_on="category", how="left", suffixes=["", "_fc"])
...
def complete_fee_audit_amazon(
	...
	req_fee_types: list=["Principal", "Product Tax", "FBA Weight Handling Fee", "Fixed closing fee", "FBA Pick & Pack Fee", "Commission", "Technology Fee", "TechnologyFee", "Refund commission", "RemovalComplete", "Amazon Easy Ship Charges", "Storage"],
	oms_name: str="EZ",
	...
	data_dict=forward_fill_settlement_date(data_dict, cols_to_ffill=["settlement-start-date", "settlement-end-date", "deposit-date"])
