...
def clean_cogs(df: pd.DataFrame()) -> dict:
    """
    ...
    for m in months.values:
        ...
        for k in pure_all_overheads:
            total_amount = payments_m.loc[
                payments_m["amount-description"] == k, "amount"
            ].sum()
...
def map_payments(
    ...
    Arguments:
        ...
        i. mkt: Boolean flag to determine whether we should use the marketing amount from payments or a separate marketing file will be provided. False = use marketing cost from payments file
    ...
    pure_order_overheads_df = pd.pivot_table(
        payment_df[payment_df["amount-description"].isin(pure_order_overheads)],
        columns="amount-description",
        index="order-id",
    ...
    mapped_orders[list(overlap_cols)] = mapped_orders[list(overlap_cols)].add(
        all_overlap_df[list(overlap_cols)], fill_value=0
    )  ## For amount-descriptions which fall in multiple classifications, we add the columns
    mapped_orders = mapped_orders.merge(
        all_overheads_df, on="key", how="left", suffixes=["", "_overheads"]
    )
